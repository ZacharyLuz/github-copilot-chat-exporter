...
function Update-ChatIndex {
    ...
    $newEntry = [PSCustomObject]@{
        ...
        mode          = if ($SessionMeta.PSObject.Properties['Mode']) { $SessionMeta.Mode } else { '' }
        workspace     = $workspace
        turns         = $SessionMeta.RequestCount
        file          = $relativePath
        exported      = $true
        firstRequest  = if ($SessionMeta.PSObject.Properties['FirstRequestTime']) { $SessionMeta.FirstRequestTime } else { '' }
        lastRequest   = if ($SessionMeta.PSObject.Properties['LastRequestTime']) { $SessionMeta.LastRequestTime } else { '' }
    ...
    [void]$mdSb.AppendLine()
    [void]$mdSb.AppendLine('| Date | Topic | Model | Mode | Workspace | Turns |')
    [void]$mdSb.AppendLine('|------|-------|-------|------|-----------|-------|')

    foreach ($entry in $indexEntries) {
        $link = "[$($entry.title)]($($entry.file))"
        $entryMode = if ($entry.mode) { $entry.mode } else { '-' }
        [void]$mdSb.AppendLine("| $($entry.date) | $link | $($entry.model) | $entryMode | $($entry.workspace) | $($entry.turns) |")
    }
