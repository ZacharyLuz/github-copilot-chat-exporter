...
function Save-GitHubCopilotChat {
    ...
    # ⚠️ UPDATE THIS PATH to match where you cloned/extracted the files
    & "C:\repos\github-copilot-chat-exporter\Save-CopilotChat.ps1" @PSBoundParameters
}
...
function Resume-GitHubCopilotChat {
    ...
    # ⚠️ UPDATE THIS PATH to match where you cloned/extracted the files
    $sessionsPath = "C:\repos\github-copilot-chat-exporter\sessions"
...
**Example paths:**
- Standard: `C:\repos\github-copilot-chat-exporter\`
- Custom location: `D:\Tools\github-copilot-chat-exporter\`
- OneDrive (not recommended for GitHub repos): `$env:USERPROFILE\OneDrive\...`
...
**Problem:** `❌ Script not found: <path>\Save-CopilotChat.ps1`
...
2. Or create it manually:
   ```powershell
   New-Item -ItemType Directory -Path "C:\repos\github-copilot-chat-exporter\sessions" -Force
   ```
